...
describe("lowerAuthoringDeclarationFile", () => {
  it("lowers a direct provenanced root call without executing throwing builders", () => {
    const sourceText = [
      'import { definePresentation as presentation, stringProp as string, after } from "@unframe/unframe-authoring";',
  ...
  it("preserves UTF-16 offsets and line columns after multiline emoji text", () => {
    const sourceText = [
      'import { definePresentation, stringProp } from "@unframe/unframe-authoring";',
  ...
  it("rejects unprovenanced root forms and root builders nested as values", () => {
    for (const sourceText of [
      ...
      'import * as presentation from "@unframe/unframe-authoring"; export default presentation.definePresentation({});',
      'import presentation from "@unframe/unframe-authoring"; export default presentation({});',
      'import type { definePresentation } from "@unframe/unframe-authoring"; const local = (...args: unknown[]) => args; export default local({});',
      'import { definePresentation } from "@unframe/unframe-authoring"; const local = definePresentation; export default local({});',
      'import { fakeBuilder } from "@unframe/unframe-authoring"; export default fakeBuilder({});',
  ...
  it("preserves ordered multi-argument nested builder calls", () => {
    const result = lower(
      'import { definePresentation, setSurfaceState } from "@unframe/unframe-authoring"; export default definePresentation({ effect: setSurfaceState("surface", "state") });',
    );
  ...
  it("accepts every builder signature class", () => {
    for (const sourceText of [
      ...
      'import { defineComponentManifest } from "@unframe/unframe-authoring"; export default defineComponentManifest({});',
      'import { defineComponentStructure } from "@unframe/unframe-authoring"; export default defineComponentStructure({});',
      'import { definePresentation, state } from "@unframe/unframe-authoring"; export default definePresentation({ value: state() });',
      'import { definePresentation, stringProp } from "@unframe/unframe-authoring"; export default definePresentation({ value: stringProp({}) });',
