...
const isPresentationBuilder = (provenance: PackageValueProvenance | undefined) =>
  provenance?.packageName === "@unframe/unframe-authoring" && provenance.subpath === ".";
...
export const lowerAuthoringDeclarationFile = (
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (
      ...
      statement.moduleSpecifier.text !== "@unframe/unframe-authoring" ||
      ...
      report(
        ...
        "Declaration files may import verified named builders from @unframe/unframe-authoring only.",
      );
      continue;
    }
    for (const element of statement.importClause.namedBindings.elements) {
