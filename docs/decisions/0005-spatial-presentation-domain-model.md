...
- Position、Rotation、Scale の物理的な意味と単位はアプリケーション間で統一する。配列かオブジェクトか、Euler 角か Quaternion かといった直列表現は API 契約の更新時に確定する。
...
- **Negative**: Group、Slide、API manifest の関係を確定し、既存の二系統のモデルを統合する移行作業が必要になる。
...
- [ ] Group と現行 Slide の関係、および保存 API と MR manifest の移行境界を決定する。
- [ ] ID のスコープ、参照整合性、`active` と `visible` の意味、Asset 参照位置を契約で確定する。
- [ ] 座標系の handedness、軸、単位、Rotation 表現、Presentation Origin の calibration を決定する。
