...
> **注記**: 本 ADR の技術選定（Go / Huma / Chi / sqlc / Turso / R2 / OpenAPI 生成）は後続設計の履歴です。ディレクトリ命名（`apps/backend` / `apps/app` / `apps/site` / `apps/mr`）は [ADR-0004](../0004-monorepo-layout-and-nix-toolchain.md) の `app/server` / `app/web` / `lp` / `app/unity` に置換され、ツールチェインは Nix flake へ移行しました。現行の目標構成は [`ARCHITECTURE.md`](../../../ARCHITECTURE.md) を参照してください。
