...
- **Status**: Proposed（比較枠組みの承認対象。PoC profile と採用結果は未確定）
- **Date**: 2026-09-02
...
この文書は、baked-web、Video、native-ui、embedded-web を同じ要件と測定条件で比較し、Issue #77 で採用範囲と fallback を決めるための判定手順を定義する。比較枠組みの承認、PoC profile の確定、方式の採否決定は別のマイルストーンとし、この文書の承認によって具体的な入力値や採用結果は確定しない。
...
Issue #73 と #76 の担当者は、Issue #78 の開始前に次を一つの PoC profile として確定する。profile の具体値は比較枠組みの承認対象に含めない。未確定欄が一つでもあれば性能の採否判定を行わず、測定結果を参考値として扱う。
...
1. **Requirement**: #73 の scene から比較対象に選んだ Surface の内容と状態遷移を簡略化せず再現できる。
...
| 順序 | マイルストーン     | 完了条件                                                        |
| ---- | ------------------ | --------------------------------------------------------------- |
| 1    | 比較枠組みの承認   | 11.1 の条件を満たし、この文書の比較手順を固定する               |
| 2    | PoC profile の確定 | #73 / #76 の入力を確定し、11.2 の開始条件を満たす               |
| 3    | #78 の実機測定     | raw result、差分画像、無効試行、未検証項目を 8 節の形式で揃える |
| 4    | 測定結果の review  | visual 差分と各 gate の pass / fail、その根拠を確認する         |
| 5    | #77 の採否決定     | Surface class ごとの採用範囲、fallback、撤退理由を記録する      |

各マイルストーンの期限は関連 Issue で管理し、先行する完了条件を満たしてから次へ進む。採否決定までに必須入力または証跡が揃わない候補は未検証として採用候補から外す。全候補が外れた場合は no-pass と記録し、未検証方式を自動採用しない。

## 10. Issue #77 の決定記録に必要な内容

Issue #77 の決定では、少なくとも次を ADR または同等の記録へ残す。
...
## 11. 承認、PoC 開始、採否決定の条件

### 11.1 比較枠組みの承認条件

- 候補方式の責務、現行実装、Target contract、PoC 専用実装の境界が区別されている。
- PoC profile の入力項目、共通 fixture、測定手順、採用 gate、出力形式が定義されている。
- Surface class ごとに gate、採用範囲、fallback、撤退理由を追跡できる。

2 節の具体値は比較枠組みの承認条件に含めない。

### 11.2 PoC 開始条件

- 2 節の入力表がすべて確定済みまたは根拠を伴う `N/A` になっている。
- Issue #78 の担当者が確定した profile から同一 fixture と測定手順を再現できる。

### 11.3 Issue #77 の採否決定条件

- Issue #78 の結果が 8 節の形式で揃い、各 gate の pass / fail と未検証項目を追跡できる。
- Issue #77 の担当者が結果から Surface class ごとの適用範囲、fallback、撤退理由を説明できる。
