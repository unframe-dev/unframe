...
public sealed class ScenePermissionGate : MonoBehaviour
{
    ...
    private void RequestNextPermission()
    {
        if (pendingPermissions.Count == 0)
        {
            ...
            Debug.Log("[ScenePermissionGate] Scene permissions granted.");
            return;
        }

        requestInFlight = true;
        var permission = pendingPermissions.Dequeue();
        var callbacks = new UnityEngine.Android.PermissionCallbacks();

        callbacks.PermissionGranted += _ =>
        {
            Debug.Log($"[ScenePermissionGate] Permission granted: {permission}");
            RequestNextPermission();
        };
        ...
        callbacks.PermissionDenied += _ =>
        {
            requestInFlight = false;
            SetSystemsEnabled(false);
            Debug.LogWarning($"[ScenePermissionGate] Permission denied: {permission}");
        };
        ...
        callbacks.PermissionDeniedAndDontAskAgain += _ =>
        {
            requestInFlight = false;
            SetSystemsEnabled(false);
            Debug.LogWarning($"[ScenePermissionGate] Permission denied permanently: {permission}");
        };

        Debug.Log($"[ScenePermissionGate] Requesting permission: {permission}");
