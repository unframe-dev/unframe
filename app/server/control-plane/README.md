...
Venue Edge credentialはprovisioning / rotation responseで一度だけ返し、D1にはtoken IDとSHA-256 hashだけを保存します。`0009_runtime_assignments.sql` は旧 Edge assignment の lease を解放し、既存 Edge が provisioning identity とは別の `runtimeId` で再登録できる状態へ移行します。migration は session state を変更しないため、適用前に `Presenting` session を停止・drainし、適用後に Edge の再登録と必要な session の再割り当てを行ってください。移行行の `runtime_id` に残る旧 `edge_id` は released history の識別子であり、active Runtime identity として再利用しません。Realtime credential署名にはEd25519 private JWKのJSONを`REALTIME_SIGNING_JWK`へ設定し、公開鍵の識別子を`REALTIME_SIGNING_KID`で管理します。`REALTIME_AUDIENCE`はRuntime JWT audienceとして必須です。値はCloud Runtimeのidentity設計と合わせて未決定のため、`wrangler.toml`の`[vars]`には固定せず環境ごとのdeployment bindingとして設定してください。Quest向けJWTはruntime ID / kind、epoch、Presentation ID / revisionを拘束し、active leaseの期限を越えて発行しません。Venue Edge assignmentだけが`assets:read`を含み、Cloud assignmentは`realtime:connect`だけを含みます。`SERVICE_IDENTITY_SECRET`はRealtime Backendからcheckpoint / completion callbackを送るための専用Bearer secretで、user session tokenやEdge固有credentialとは共有しません。秘密値はremote環境ではWrangler secretとして設定してください。

`wrangler.toml` の `secrets.required` は remote 環境へ事前設定する binding の一覧です。`REALTIME_AUDIENCE` の具体値は Runtime identity contract と合わせて決定するまで追跡対象の設定へ固定せず、ほかの secret と同じ deployment binding として設定してください。Worker は request handling の入口で全設定を検証するため、不足・不正な設定がある環境は traffic を受ける前の smoke test で検出します。エラーには設定名だけを出し、値は出力しません。
